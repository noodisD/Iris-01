...
def test_complete_journal_entry_flow(test_user, mocker):
    ...
    entry_id = db.create_reflection(test_user['id'], content)

    # 3. Verify initial state
    items = db.get_items_to_process('reflection', status='pending', limit=1)
    assert len(items) > 0
    # A reflection is embedded inside its header; the owner's words are within it.
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT processing_status FROM reflections WHERE id = %s;", (entry_id,))
...
def test_batch_processing_flow(test_user, mocker):
    ...
    entry_ids = []
    for i in range(3):
        entry_id = db.create_reflection(test_user['id'], f'Entry {i}: Test content {i}')
...
def test_error_handling_in_pipeline(test_user, mocker):
    ...
    # Create entry
    entry_id = db.create_reflection(test_user['id'], 'Entry that will fail')
...
def test_idempotent_processing(test_user, mocker):
    ...
    # Create entry
    entry_id = db.create_reflection(test_user['id'], 'Entry to process multiple times')
    ...
    with conn.cursor() as cur:
        cur.execute(
            "SELECT processing_status FROM reflections WHERE id = %s;",
            (entry_id,)
        )
        status = cur.fetchone()[0]
        assert status == 'complete'
...
def test_data_recovery_from_postgres(test_user, mocker):
    ...
    entry_id = db.create_reflection(test_user['id'], 'Important entry')
    run_processing_pipeline('reflection', entry_id)
...
def test_processing_status_transitions(test_user):
    ...
    """
    entry_id = db.create_reflection(test_user['id'], 'Status tracking entry')
    ...
    db.update_processing_status('reflection', entry_id, 'processing')
    with conn.cursor() as cur:
        cur.execute("SELECT processing_status FROM reflections WHERE id = %s;", (entry_id,))
        status = cur.fetchone()[0]

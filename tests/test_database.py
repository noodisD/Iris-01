...
def test_create_reflection(test_user):
    """A journal entry is a reflection (ADR-0010)."""
    entry_id = db.create_reflection(test_user['id'], 'This is a test journal entry.')
...
def test_add_and_get_embedding(test_user):
    ...
    entry_id = db.create_reflection(test_user['id'], 'Entry to be embedded.')
    ...
    retrieved_vector = get_embedding("reflection", entry_id, model_name)
...
def test_update_and_get_processing_status(test_user):
    ...
    entry_id = db.create_reflection(test_user['id'], 'Another test entry.')

    db.update_processing_status("reflection", entry_id, "complete")
    ...
    status = get_status("reflection", entry_id)
...
def test_theme_operations(test_user):
    ...
    db.add_theme_occurrence(theme_id, 'reflection', 9001, "one", 0.9,
                            "2024-01-01T10:00:00Z")
    db.add_theme_occurrence(theme_id, 'reflection', 9002, "two", 0.9,
...
def test_theme_occurrence_operations(test_user):
    ...
    entry_id = db.create_reflection(test_user['id'], 'I keep thinking about this idea.')
...
def test_unassigned_embeddings(test_user):
    ...
    entry_id = db.create_reflection(test_user['id'], 'Unassigned entry')
    vector = [0.4] * 1536
    db.add_embedding("reflection", entry_id, "test_model", vector)
    ...
    db.add_theme_occurrence(theme_id, "reflection", entry_id, "snippet", 1.0, "2024-01-01T00:00:00Z")
